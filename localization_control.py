...
class LocalizationControlError(RuntimeError):
    """Localization could not be controlled safely."""


class LocalizationConflictError(LocalizationControlError):
    """Another process already owns localization."""


class LocalizationControl:
    """Own one conservative localization-only launch session."""
    ...
    @property
    def command(self):
        """Return the fixed localization-only launch command."""
        return [
    ...
    def start(self, timestamp):
        """Start one validated localization-only session."""
        with self._lock:
            self._refresh_locked()
            ...
            if external:
                raise LocalizationConflictError(
                    'Localization or Nav2 processes already '
            ...
            if process.poll() is not None:
                self._last_error = (
                    'Localization launch exited immediately.'
                )
                raise LocalizationControlError(
                    self._last_error
                )
    ...
    def stop(self, timestamp):
        """Stop only the localization session this object owns."""
        with self._lock:
            self._refresh_locked()
            ...
            if not self._identity_checker(pid):
                self._last_error = (
                    'Owned localization PID identity changed; '
                    'no signal was sent.'
                )
                raise LocalizationControlError(
                    self._last_error
            ...
            if process_group != pid:
                self._last_error = (
                    'Localization is not its session leader; '
                    'no signal was sent.'
                )
                raise LocalizationControlError(
                    self._last_error
            ...
            try:
                ...
                try:
                    ...
                    self._last_error = (
                        'Localization did not stop after '
                        'SIGINT and SIGTERM.'
                    )
                    raise LocalizationControlError(
                        self._last_error
    ...
    @classmethod
    def _is_localization_command(cls, command):
        normalized = ' '.join(
            str(command).split()
        )
    ...
    def _find_external_processes(cls, owned_pid):
        ...
        for process_directory in Path('/proc').iterdir():
            ...
            if cls._is_localization_command(command):
                found.append(pid)
