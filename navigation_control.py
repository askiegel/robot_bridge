...
class NavigationControlError(RuntimeError):
    """Guarded navigation could not be controlled safely."""


class NavigationConflictError(NavigationControlError):
    """Another process already owns navigation."""


class NavigationControl:
    """Own one guarded navigation runtime without submitting goals."""

    ROS2_EXECUTABLE = Path('/opt/ros/humble/bin/ros2')
    PACKAGE = 'mini_pupper_navigation'
    LAUNCH_FILE = 'guarded_navigation.launch.py'
    ...
    @property
    def command(self):
        """Return the fixed guarded navigation launch command."""
        return [
    ...
    def start(self, timestamp):
        """Start one validated guarded navigation session."""
        with self._lock:
            self._refresh_locked()
            ...
            if external:
                raise NavigationConflictError(
                    'Guarded navigation or Nav2 processes already '
            ...
            if process.poll() is not None:
                self._last_error = (
                    'Guarded navigation launch exited immediately.'
                )
    ...
    def stop(self, timestamp):
        """Stop only the navigation session this object owns."""
        with self._lock:
            self._refresh_locked()
            ...
            if not self._identity_checker(pid):
                self._last_error = (
                    'Owned navigation PID identity changed; '
                    'no signal was sent.'
                )
                raise NavigationControlError(
                    self._last_error
            ...
            if process_group != pid:
                self._last_error = (
                    'Guarded navigation is not its session leader; '
                    'no signal was sent.'
                )
                raise NavigationControlError(
                    self._last_error
            ...
            try:
                ...
                try:
                    ...
                    self._last_error = (
                        'Guarded navigation did not stop after '
                        'SIGINT and SIGTERM.'
                    )
                    raise NavigationControlError(
                        self._last_error
    ...
    @classmethod
    def _is_navigation_command(cls, command):
        normalized = ' '.join(
            str(command).split()
        )
    ...
    def _find_external_processes(cls, owned_pid):
        ...
        for process_directory in Path('/proc').iterdir():
            ...
            if cls._is_navigation_command(command):
                found.append(pid)
